...
/**
 * Reactive client for fetching and parsing the backend's raw OpenAPI specification.
 * 
 * <p>This client is responsible for:</p>
 * <ul>
 *   <li>Fetching the OAS from the backend service (entity-persistence-service)</li>
 ...
 * <p>The raw OAS is cached in-memory since it's identical for all users.
 * Only the transformed/pruned specs are cached per-role in Redis.</p>
 */
@Component
@Slf4j
public class BackendOasClient {
    ...
    private void initWebClient() {
        ...
        HttpClient httpClient = HttpClient.create()
            ...
            .doOnConnected(connection -> {
                ...
                connection.addHandlerLast(new WriteTimeoutHandler(config.getWriteTimeoutMs(), TimeUnit.MILLISECONDS));
            });
     ...
     * 
     * <p>Uses in-memory caching with TTL to avoid repeated network calls.
     * The raw OAS is shared across all users.</p>
     * 
    ...
    public Mono<OpenAPI> fetchRawOas() {
        ...
        if (cached != null && !cached.isExpired()) {
            log.debug("Returning cached raw OAS (expires in {}ms)", cached.getRemainingTtlMs());
        ...
        return webClient.get()
            ...
            .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), response -> {
                ...
                return Mono.error(new ResponseStatusException(
                    ...
                    "Failed to fetch OpenAPI specification from backend"
                ));
            })
            ...
            .doOnNext(openApi -> {
                // Cache the parsed OAS
                Duration ttl = properties.getCache().getRawOasTtl();
                cachedRawOas.set(new CachedOas(openApi, ttl));
                log.info("Raw OAS cached successfully (TTL: {})", ttl);
            })
            .doOnError(e -> log.error("Failed to fetch raw OAS: {}", e.getMessage()))
            .onErrorResume(e -> {
                // Try to return stale cache on error
                CachedOas stale = cachedRawOas.get();
                if (stale != null) {
                    log.warn("Returning stale cached OAS due to fetch error");
                    return Mono.just(stale.getOpenApi());
                }
                return Mono.error(new ResponseStatusException(
                    HttpStatus.SERVICE_UNAVAILABLE,
                    "OpenAPI specification temporarily unavailable"
                ));
            });
    }
    ...
    private OpenAPI parseOasContent(String content) {
        ...
        log.debug("Successfully parsed OAS: {} paths, {} schemas",
            ...
            result.getOpenAPI().getComponents() != null && result.getOpenAPI().getComponents().getSchemas() != null
                ? result.getOpenAPI().getComponents().getSchemas().size() : 0
        );
